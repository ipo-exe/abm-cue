...
def play_deprec(df_agents, df_places, n_steps, n_beta, n_alpha, r_d_agents, r_c_places, b_trace=True):
    """
...
def play(df_agents, df_places, n_steps, b_return=False, b_trace=True):
    ...
    for t in range(1, n_steps):
        ...
        for a in range(n_agents):
            ...
            df_window['Discrepancy'] = np.abs(n_crt_agent_trait - df_window['Trait'].values)
            ...
            if df_window['Interac_score'].values[n_next_index] > 0:
                # get space parameters from window dataframe
                n_crt_place_traits = df_window['Trait'].values[n_next_index]
                n_crt_place_x = df_window['x'].values[n_next_index]
                n_crt_place_d = df_window['D'].values[n_next_index]

                # compute means
                r_mean_agent = (n_crt_agent_trait + (n_crt_agent_c * n_crt_place_traits)) / (1 + n_crt_agent_c)
                r_mean_place = (n_crt_place_traits + (n_crt_place_d * n_crt_agent_trait)) / (1 + n_crt_place_d)
